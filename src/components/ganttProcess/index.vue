  ...
  <div class="wl-gantt" id="wl-gantt">
    ...
    >
      <!-- <template slot="expandIcon" slot-scope="props">
        <template v-if="props.record.children && props.record.children.length">
          <a-icon
      ...
      <template v-if="!ganttOnly">
        ...
        >
          <template slot-scope="name, record">
            <a-input
              v-if="self_cell_edit === '_n_m_' + record[selfProps.id]"
              v-model="record[selfProps.name]"
            ...
            <strong v-else class="h-full">
              <!-- <span @click="cellEdit('_n_m_' + record[selfProps.id], 'wl-name')"> -->
              <span>
                {{ nameFormatter ? nameFormatter(record, record[selfProps.id]) : record[selfProps.name] }}
              </span>
            </strong>
          </template>
        </a-table-column>
        <a-table-column title="开始日期" key="startDate" data-index="startDate" fixed="left" width="150px">
          <template slot-scope="name, record">
            <a-date-picker
              v-if="self_cell_edit === '_s_d_' + record[selfProps.id]"
              v-model="record[selfProps.startDate]"
              @change="startDateChange(record)"
              ...
              placeholder="请选择开始日期"
            ></a-date-picker>
            <!-- <div v-else class="h-full" @click="cellEdit('_s_d_' + record[selfProps.id], 'wl-start-date')"> -->
            <div v-else class="h-full">
              {{ timeFormat(record[selfProps.startDate]) }}
            </div>
          </template>
        </a-table-column>
        <a-table-column title="结束日期" key="endDate" data-index="endDate" fixed="left" width="150px">
          <template slot-scope="name, record">
            <a-date-picker
              v-if="self_cell_edit === '_e_d_' + record[selfProps.id]"
              v-model="record.endDate"
              @change="endDateChange(record)"
              @blur="self_cell_edit = null"
              type="date"
              size="medium"
              class="u-full"
              ...
              placeholder="请选择结束日期"
            ></a-date-picker>
            <!-- <div v-else class="h-full" @click="cellEdit('_e_d_' + record[selfProps.id], 'wl-end-date')"> -->
            <div v-else class="h-full">
              {{ timeFormat(record[selfProps.endDate]) }}
            </div>
          </template>
        </a-table-column>
        <a-table-column
      ...
      <template v-if="self_date_type === 'yearAndMonth'">
        ...
        >
          ...
          >
            <template slot-scope="name, record">
              <div :class="dayGanttType(record, month.full_date, 'months')"></div>
              <div v-if="useRealTime" :class="realDayGanttType(record, month.full_date, 'months')"></div>
            </template>
          </a-table-column>
        </a-table-column-group>
        <!-- </a-table-column-group> -->
      ...
      <template v-else-if="self_date_type === 'yearAndWeek'">
        <!-- <a-table-column-group class="name-col">
          <div slot="title" style="position: absolute; width: 100%; left: 0">
            <a-slider />
          </div> -->
        ...
        >
          ...
          >
            <template slot-scope="name, record">
              <div :class="dayGanttType(record, t.full_date, 'week')"></div>
              <div v-if="useRealTime" :class="realDayGanttType(record, t.full_date, 'week')"></div>
            </template>
          </a-table-column>
        </a-table-column-group>
        <!-- </a-table-column-group> -->
      ...
      <template v-else>
        ...
        >
          ...
          >
            <template slot-scope="name, record">
              <div :class="dayGanttType(record, t.full_date)"></div>
              <div v-if="useRealTime" :class="realDayGanttType(record, t.full_date)"></div>
            </template>
          </a-table-column>
        </a-table-column-group>
        <!-- </a-table-column-group> -->
...
import { UUID } from '@/utils/custom/util'
import isBetween from 'dayjs/plugin/isBetween'
dayjs.extend(isBetween)
import { flattenDeep, getMax, flattenDeepParents, regDeepParents } from '@/utils/core' // 导入数组操作函数
...
export default {
  ...
  computed: {
    ...
    ganttTitleDate() {
      ...
      if (this.autoGanttDateType) {
        ...
        console.log(mouth_diff, '--mouth_diff')
        if (mouth_diff > 12) {
  ...
  methods: {
    ...
    endDateChange(row) {
      // 如果开始晚于项目结束，则把项目结束延后
      let _early_project_end = this.timeIsBefore(this.self_end_date, row[this.selfProps.endDate])
    ...
    handlePreTask(item) {
      // 统一在一维化数据中处理前置任务
      let _pre_target = this.self_dependent_store.find((i) => i.form[this.selfProps.id] === item[this.selfProps.id])
